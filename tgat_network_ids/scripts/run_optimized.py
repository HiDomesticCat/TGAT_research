...
try:
    ...
    # 導入優化版模組
    from tgat_network_ids.src.data.optimized_data_loader import EnhancedMemoryOptimizedDataLoader
    from tgat_network_ids.src.data.optimized_graph_builder import OptimizedDynamicNetworkGraph
    from tgat_network_ids.src.models.optimized_tgat_model import OptimizedTGAT
    ...
    # 導入記憶體優化工具
    sys.path.insert(0, os.path.join(parent_dir, 'tgat_network_ids'))
    ...
    from src.data.optimized_graph_builder import OptimizedDynamicNetworkGraph
    from src.models.optimized_tgat_model import OptimizedTGAT
    from src.models.memory_optimized_train import MemoryOptimizedTGATTrainer
    from src.visualization.visualization import NetworkVisualizer
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("optimized_tgat_ids.log"),
...
def parse_args():
    """解析命令行參數"""
    parser = argparse.ArgumentParser(description='高效記憶體優化版 TGAT 網路攻擊檢測系統')
    ...
    parser.add_argument('--limit_gpu_memory', type=int, default=0,
                        help='限制GPU記憶體使用量(MB), 0表示不限制')
    parser.add_argument('--use_polars', action='store_true', default=False,
                        help='使用Polars加速資料處理')
    ...
    parser.add_argument('--use_gradient_checkpointing', action='store_true', default=False,
                        help='使用梯度檢查點')
    parser.add_argument('--use_sparse_representation', action='store_true', default=True,
                        help='使用稀疏表示')
...
def get_config(args):
    ...
    if args.use_polars:
        config['data']['use_polars'] = True
        
    # 圖相關配置
    if 'graph' not in config:
        config['graph'] = {}
        
    if args.use_sparse_representation:
        config['graph']['use_sparse_representation'] = True
    ...
    if args.use_mixed_precision:
        config['model']['use_mixed_precision'] = True
...
def load_and_preprocess_data(config):
    """加載和預處理資料 - 使用優化資料載入器"""
    logger.info(f"使用優化資料載入器加載資料集: {config['data']['path']}")
...
def build_graph(config, data_loader, features, labels, indices=None):
    """建立圖結構 - 使用優化圖建構器"""
    # 初始化優化圖建構器
    graph_builder = OptimizedDynamicNetworkGraph(config)
    ...
    if edges is not None and len(edges) > 0:
        ...
        # 如果沒有邊，使用 simulate_stream 方法創建邊
        logger.info("未獲取到時間性邊，使用 simulate_stream 方法創建邊")
        # 創建時間戳記 (使用索引作為時間戳記)
    ...
    # 使用稀疏張量表示進一步優化記憶體
    if config['graph'].get('use_sparse_representation', True):
        sparse_tensor = graph_builder.to_sparse_tensor()
        logger.info("已轉換為稀疏張量表示")
...
def train_model(config, graph, labels, val_graph=None, val_labels=None, class_names=None):
    ...
    if model_config.get('use_gradient_accumulation', False):
        steps = model_config.get('gradient_accumulation_steps', 4)
        model.enable_gradient_accumulation(steps)
        
    if model_config.get('use_sparse_gradients', False):
        model.enable_sparse_gradients()
    ...
    logger.info(f"  梯度檢查點: {model_config.get('use_gradient_checkpointing', False)}")
...
def evaluate_model(trainer, graph, labels, class_names=None, visualize=False, config=None):
    ...
    # 如果需要，將模型轉換為稀疏表示以減少記憶體使用
    if hasattr(trainer.model, 'to_sparse_tensor'):
        trainer.model.eval()  # 設為評估模式
        trainer.model.to_sparse_tensor()

    # 評估模型
    loss, acc, metrics = trainer.evaluate(graph, labels, class_names)

    # 將模型恢復為密集表示（如果之前轉換了）
    if hasattr(trainer.model, 'to_dense_tensor'):
        trainer.model.to_dense_tensor()
...
def simulate_real_time_detection(config, data_loader, model, device='cpu', visualize=False):
    ...
    # 初始化優化圖建構器
    graph_builder = OptimizedDynamicNetworkGraph(config, device=device)
    ...
    for batch_idx in range(num_batches):
        logger.info(f"處理批次 {batch_idx+1}/{num_batches}")
        ...
        # 如果需要在推理階段轉換為稀疏表示以節省記憶體
        if hasattr(model, 'to_sparse_tensor'):
            model.to_sparse_tensor()
        ...
        # 如果之前轉換為稀疏表示，現在恢復
        if hasattr(model, 'to_dense_tensor'):
            model.to_dense_tensor()
...
def main():
    ...
    # 限制GPU記憶體使用量（如果指定）
    if args.limit_gpu_memory > 0 and device != 'cpu':
        limit_gpu_memory(args.limit_gpu_memory, args.gpu)
        logger.info(f"已限制GPU記憶體使用量為 {args.limit_gpu_memory} MB")
    ...
    if args.monitor_memory:
        ...
        memory_monitor = MemoryMonitor(
            ...
            alert_threshold=80.0  # 使用更低的閾值提前發出警告
        )
    ...
    try:
        ...
        if args.mode == 'train':
            ...
            if config['model'].get('use_mixed_precision', False):
                model.enable_mixed_precision()
            ...
            logger.info(f"已載入預訓練模型: {model_path}")
            ...
            # 載入模型 - 使用與測試模式相同的程序
            # 獲取特徵維度
            # 這裡我們使用部分訓練數據來獲取特徵維度，而不是所有測試數據
            sample_data = X_train.iloc[:1000] if len(X_train) > 1000 else X_train
            in_dim = sample_data.shape[1]
            ...
            if len(detected_attacks) > 0:
                ...
                result_path = os.path.join(
                    config['output']['result_dir'],
                    f'detection_results_{get_timestamp()}.json'
                )
                save_results(detection_results, result_path)
                logger.info(f"已保存檢測結果至: {result_path}")
...
if __name__ == "__main__":
    ...
    logger.info(f"總執行時間: {int(hours)}小時 {int(minutes)}分鐘 {seconds:.2f}秒")
